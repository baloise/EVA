...
if($session_usergroup == 1 || $session_usergroup == 3 || $session_usergroup == 4 || $session_usergroup == 5){
    ...
    function calculateSubject($subjectID, $mysqli){
        ...
        $sql = "SELECT grade, weighting, title FROM `tb_subject_grade` WHERE tb_user_subject_ID = $subjectID";
        $gradesList = fetchColumn($mysqli->query($sql));
        $weights = fetchColumn($mysqli->query($sql), 1);
        $title = fetchColumn($mysqli->query($sql), 2);
        ...
        if(count($grades) > 0){
            ...
            $chooseMaxPoints = "SELECT weight, subjectName FROM `tb_user_subject` WHERE ID = $subjectID";
            $resultchoose = $mysqli->query($chooseMaxPoints);
            $rowchoose = $resultchoose->fetch_assoc();
            ...
            */
            ...
            if(strpos($title[0], '[DONTROUND]') !== false){
                $calcGradeRounded = (array_sum($grades) / array_sum($weights));
            } else {
