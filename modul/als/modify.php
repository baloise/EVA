<?php
    ...
    if($_POST['todo'] == "checkAndDelete"){
		...
		if($error){
			...
			if($result->num_rows != 1){
                ...
                $stmt = $mysqli->prepare("SELECT tb_user_ID, title FROM `tb_als` WHERE ID = ?;");
                ...
                $subject = strtr($translate[220], $msgcontent);
                $message = strtr($translate[221], $msgcontent);
                sendMail($subject, $message, $session_userid, $userInfo[0], $session_appinfo, $mysqli, $translate);

				$stmt = $mysqli->prepare("DELETE FROM `tb_als` WHERE `tb_als`.`ID` = ?");
				$stmt->bind_param("i", $entryid);
				$stmt->execute();
    ...
    if($_POST['todo'] == "check"){
		...
		if($error){
			...
			if($result->num_rows != 1){
                ...
                include("../../modul/mail/generateMail.php");
                $msgcontent = array('{title}' => $userInfo[1], '{reason}' => $reason);
                $subject = strtr($translate[222], $msgcontent);
                $message = strtr($translate[223], $msgcontent);
                sendMail($subject, $message, $session_userid, $userInfo[0], $session_appinfo, $mysqli, $translate);
    ...
    if($_POST['todo'] == "addEntry"){
        ...
        if(!isset($title)){
            $error = $error . "Bitte ALS-Titel angeben.<br/>";
        }
        ...
        if($error){
            ...
            $subject = strtr($translate[224], $msgcontent);
            $message = strtr($translate[225], $msgcontent);
